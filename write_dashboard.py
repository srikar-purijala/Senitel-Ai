...
dashboard_content = '''import { useEffect, useState } from 'react';
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#d2d2d7]/50 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 max-h-[220px]">
            {liveEvents.slice(0, 15).map(ev => (
              <div key={ev.id} className={px-3 py-2 rounded-xl text-[12px] flex items-start gap-2 }>
...
# Fix interpolation syntax for python string formatting vs JS template literals
dashboard_content = dashboard_content.replace('', '$\\{String(i).padStart(2, \'0\')\\}')
dashboard_content = dashboard_content.replace('', '$\\{(exposureDetected / 100000).toFixed(1)\\}')
dashboard_content = dashboard_content.replace('', '$\\{m.urgent ? \'border-red-200 bg-red-50\' : \'border-[#d2d2d7]/50\'\\}')
dashboard_content = dashboard_content.replace('', '$\\{ev.type === \'CRITICAL\' ? \'bg-red-50 border border-red-100\' : ev.type === \'WARNING\' ? \'bg-amber-50 border border-amber-100\' : \'bg-[#f5f5f7] border border-[#d2d2d7]/50\'\\}')
dashboard_content = dashboard_content.replace('', '$\\{ev.type === \'CRITICAL\' ? \'bg-red-500\' : ev.type === \'WARNING\' ? \'bg-amber-500\' : \'bg-[#aeaeb2]\'\\}')
dashboard_content = dashboard_content.replace('', '$\\{ev.type === \'CRITICAL\' ? \'text-red-700\' : ev.type === \'WARNING\' ? \'text-amber-700\' : \'text-[#424245]\'\\}')

# Wait, if I use raw python strings I don't need to escape the JS template variables.
# The issue was I used Powershell variable substitution!
# So I'll write the raw string directly to file.

with open('frontend/src/pages/Dashboard.tsx', 'w', encoding='utf-8') as f:
    f.write(dashboard_content.replace('\\{', '{').replace('\\}', '}'))
